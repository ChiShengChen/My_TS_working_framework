...
# We will use the `Dataset` class to load the data and run the model. If you have not already please check out the [dataset.ipynb](./dataset.ipynb) notebook to learn more about the `Dataset` class. We are going to just run the model on two datasets for brevity. 
...
# We are going to follow the naming conventions explained in the [README](../README.md) file to store the results in a csv file called `all_results.csv` under the `results/timesfm_2_0_500m` folder.
...
### Ordering all dataset settings from lowest to highest prediction length to minimize the number of jittings. 
# This is not necessary for the pytorch version.
import csv
import os
...
## Results
# Running the above cell will generate a csv file called `all_results.csv` under the `results/timesfm` folder containing the results 
